...
export default function CartSidebar() {
  ...
  return (
    <div className="fixed inset-0 z-[90] flex items-end sm:items-stretch sm:justify-end">
      ...
      <div className="relative w-full sm:w-[min(430px,50vw)] bg-cream flex flex-col shadow-2xl anim-sheet-up rounded-t-[2rem] sm:rounded-none h-[85vh] sm:h-full border-t-2 sm:border-t-0 sm:border-l-2 border-brand">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b-2 border-brand/15 shrink-0">
          ...
          <div className="flex items-center gap-2">
            ...
            <button onClick={() => closeCart()} className="w-9 h-9 rounded-full border-2 border-brand bg-transparent hover:bg-accent transition flex items-center justify-center">
            ...
            cart.map((item, idx) => (
              <div key={item.key} className="flex items-start gap-3 p-4 rounded-2xl border-2 border-brand/20 bg-brand/5 cart-item-in">
                ...
                <div className="flex items-center gap-2 shrink-0">
                  <button onClick={() => stepItem(idx, -1)} className="w-8 h-8 rounded-full border-2 border-brand bg-transparent font-bold hover:bg-accent transition flex items-center justify-center text-sm">–</button>
                  <span className="min-w-[22px] text-center font-extrabold text-base" style={{ fontFamily: 'var(--font-baloo)' }}>{item.qty}</span>
                  <button onClick={() => stepItem(idx, 1)} className="w-8 h-8 rounded-full bg-brand text-cream font-bold hover:bg-accent hover:text-brand transition flex items-center justify-center text-sm">+</button>
                </div>
              </div>
            ))
          )}
        ...
        <div className="px-5 pb-4 sm:pb-6 pt-4 border-t-2 border-brand/15 space-y-3 shrink-0 safe-pb">
